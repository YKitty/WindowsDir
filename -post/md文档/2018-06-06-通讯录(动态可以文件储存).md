# 通讯录(动态可以文件储存)

___

分为三个部分.address.h   address.c   test.c

## address.h

```c
#pragma once //确保头文件被引用一次，但是可移植性差   
//最好用#ifndef #defiene #endif 可移植性高  
//#ifndef __ADDRESS_H__ //防止头文件被多次定义，该头文件是  address.h  
//#define __ADDRESS_H__  


//检测X86是否被宏定义，如果没有的话在编译输出窗口输出X86 not define  
//#ifndef X86  
//  
//#pragma message("X86 not define")  
//  
//#endif  

#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#define FILENAME "linkman.txt"

enum INPUT //定义一个枚举类型  
{
	A, //枚举常量  
	ENTER,
	EMPTY,
	DELETE,
	SORT,
	SHOW,
	SEARCH,
	REVISE, //修改信息  
	EXIT
};

typedef struct LinkMan
{
	char name[20];
	char sex[4];
	int age;
	char tel[15];
	char addr[50];
}LinkMan;

typedef struct LinkManCount
{
	LinkMan *contact;//初始化可以放三个联系人
	int sz;//记录当前存了几个联系人
	int max;//记录当前最多可以放几个联系人
}LMCount, *pLMCount;

void load_linkman(pLMCount my_addr);
void save_linkman(pLMCount my_addr);
void init_linkman(pLMCount my_addr);
void enter_linkman(pLMCount my_addr);
void show_linkman(pLMCount my_addr);
void delete_linkman(pLMCount my_addr);
void empty_linkman(pLMCount my_addr);
void sort_linkman(pLMCount my_addr);
void search_linkman(pLMCount my_addr);
void revise_linkman(pLMCount my_addr);


//#endif //__ADDRESS_H__

```

## address.c

``` c
#include "address.h"

//static保证这个函数的安全，改变了链接属性，不被其他文件所使用
static void allocate_mem(pLMCount my_addr)//自己判断内存空间是否足够，如果内存空间不够，那么就在追加两个LinkMan类型的空间
{
	if (my_addr->sz == my_addr->max)//内存已经使用完了
	{
		//每次扩大两个LinkMan类型的空间
		//注意函数realloc函数的使用，第一个参数为动态内存的起始地址
		LinkMan *p = (LinkMan *)realloc(my_addr->contact, ((my_addr->max) += 2) * sizeof(LinkMan));
		if (p != NULL)
		{
			my_addr->contact = p;
		}
		printf("Allocate memory success.\n");
	}
	else
	{
		return;
	}
}

//加载文件
void load_linkman(pLMCount my_addr)
{
	FILE* pfinit = fopen(FILENAME, "r");
	LinkMan tmp = { 0 };
	if (pfinit == NULL)
	{
		perror("fopen the file for init");
		exit(EXIT_FAILURE);
	}

	//加载时先要判断空间是否充足，然后再将文件内容加载进去
	while (fread(&tmp, sizeof(LinkMan), 1, pfinit))
	{
		allocate_mem(my_addr);
		my_addr->contact[my_addr->sz] = tmp;
		my_addr->sz++;
	}

	fclose(pfinit);
	pfinit = NULL;
}

//初始化通讯录
void init_linkman(pLMCount my_addr)
{
	my_addr->sz = 0;
	my_addr->max = 3; //开始存放三个联系人
	my_addr->contact = (LinkMan *)calloc(sizeof(LinkMan), my_addr->max);//动态分配空间并且初始化为零
	if (my_addr->contact == NULL)
	{
		printf("Not able to allocate memory!");
		exit(1);//exit函数后面除了0以外全都是异常使程序停止
	}
	//每次初始化的时候要加载已有的联系人信息
	load_linkman(my_addr);
}

//输入联系人信息
void enter_linkman(pLMCount my_addr)
{
	allocate_mem(my_addr);
	

	//if (my_addr->sz == my_addr->max)//内存已经使用完了
	//{
	//	allocate_mem(my_addr);
	//	printf("Allocate memory success.\n");
	//}
	printf("Please scanf name:>");
	scanf("%s", my_addr->contact[my_addr->sz].name);
	printf("Please sacnf sex:>");
	scanf("%s", my_addr->contact[my_addr->sz].sex);
	printf("Please sacnf age:>");
	scanf("%d", &my_addr->contact[my_addr->sz].age);
	printf("Please sacnf tel:>");
	scanf("%s", my_addr->contact[my_addr->sz].tel);
	printf("Please sacnf addr:>");
	scanf("%s", my_addr->contact[my_addr->sz].addr);
	printf("Enter Over.\n");
	my_addr->sz++;
}

//展示联系人信息
void show_linkman(pLMCount my_addr)
{
	if (my_addr == NULL)
	{
		printf("Not have linkman,please enter linkman again.\n");
		return;
	}
	if (my_addr->sz == 0)
	{
		printf("Not have linkman to show.\n");
		return;
	}
	else
	{
		int i = 0;
		printf("name\tsex\tage\ttel\taddr\n");
		for (i = 0; i < my_addr->sz; i++)
		{
			printf("%-s\t%-s\t%-d\t%-s\t%-s\n", my_addr->contact[i].name,
				my_addr->contact[i].sex,
				my_addr->contact[i].age,
				my_addr->contact[i].tel,
				my_addr->contact[i].addr);
		}
	}
}

//按名字查找联系人
//static改变该函数的链接属性，使其只能在该源文件下可以使用
static int find_people_name(pLMCount my_addr, char *deal_name)
{
	int i = 0;
	for (i = 0; i < my_addr->sz; i++)
	{
		if (!(strcmp(my_addr->contact[i].name, deal_name)))//判断名字是否相同
		{
			return i;  //找到返回下标
		}
	}
	return -1;//没有找到返回-1
}

//删除联系人信息
void delete_linkman(pLMCount my_addr)
{
	char del_name[20] = "0";
	int ret = 1;
	int i = 0;
	if (my_addr == NULL)//表示还没开始添加联系人
	{
		printf("Not have linkman,please enter linkman again.\n");
		return;
	}
	if (my_addr->sz == 0) //表示将联系人清空之后的情况，my_addr的地址存在，但是没有联系人了
	{
		printf("Not have linkman to delete.\n");
		return;
	}
	printf("Please scanf name that you want delete:>");
	scanf("%s", del_name);
	while (ret)//可以删除名字重复的所有联系人
	{
		ret = find_people_name(my_addr, del_name);
		if (ret != -1)
		{
			for (i = ret; i < my_addr->sz - 1; i++)//删除名字为del_name的联系人
			{
				my_addr->contact[i] = my_addr->contact[i + 1];
			}
			my_addr->sz--;
			if (ret == 0)//如果第一个删除的联系人的下标为0，将其下标改为1
			{
				ret = 1;
			}
		}
		else //没有找到联系人，那么删除结束
		{
			ret = 0;
		}
	}
	printf("Delete Over.\n");
}

//保存联系人到文件中
void save_linkman(pLMCount my_addr)
{
	int i = 0;
	FILE* pfwrite = fopen(FILENAME, "w");
	if (pfwrite == NULL)
	{
		perror("write the linkman to FILENAME");
		exit(EXIT_FAILURE);
	}

	for (i = 0; i < my_addr->sz; i++) //将联系人写入到文件中
	{
		fwrite(my_addr->contact + i, sizeof(LinkMan), 1, pfwrite);
	}

	fclose(pfwrite);
	pfwrite = NULL;
}

//清空联系人
void empty_linkman(pLMCount my_addr)
{
	//保存联系人数据
	save_linkman(my_addr);
	LinkMan *p = (LinkMan *)calloc(sizeof(LinkMan) , (my_addr->max));//将其进行从新开辟空间，便是初始化，，哈哈
	if (p != NULL)
	{
		my_addr->contact = p;
	}
	my_addr->sz = 0;
	printf("Empty Over.\n");
}

//对联系人进行排序
void sort_linkman(pLMCount my_addr)
{
	//对姓名进行排序
	int i = 0;
	int j = 0;
	for (i = 0; i < my_addr->sz - 1; i++)
	{
		for (j = 0; j < my_addr->sz - 1 - i; j++)
		{
			if (strcmp(my_addr->contact[j].name, my_addr->contact[j + 1].name) > 0)
			{
				LinkMan tmp = my_addr->contact[j];
				my_addr->contact[j] = my_addr->contact[j + 1];
				my_addr->contact[j + 1] = tmp;
			}
		}
	}
	printf("Sort Over.\n");
}

//对联系人进行查找并且输出
void search_linkman(pLMCount my_addr)
{
	int i = 0;
	int ret = -1; //存放要查找的联系人下标
	char search_name[20] = "0";
	if (my_addr == NULL)//表示还没开始添加联系人
	{
		printf("Not have linkman,please enter linkman again.\n");
		return;
	}
	if (my_addr->sz == 0) //表示将联系人清空之后的情况，my_addr的地址存在，但是没有联系人了
	{
		printf("Not have linkman to search.\n");
		return;
	}
	printf("Please scanf name that you want search:>");
	scanf("%s", search_name);
	ret = find_people_name(my_addr, search_name);  //首先将第一个找到的联系人进行输出
	if (ret != -1)
	{
		printf("name\tsex\tage\ttel\taddr\n");
		printf("%-s\t%-s\t%-d\t%-s\t%-s\n", my_addr->contact[i].name,
			my_addr->contact[i].sex,
			my_addr->contact[i].age,
			my_addr->contact[i].tel,
			my_addr->contact[i].addr);
	}
	for (i = ret + 1; i < my_addr->sz; i++)//将通讯录所有叫该名字的全部输出			
	{
		if (!(strcmp(my_addr->contact[i].name, search_name)))//判断名字是否相同
		{
			printf("%-s\t%-s\t%-d\t%-s\t%-s\n", my_addr->contact[i].name,
				my_addr->contact[i].sex,
				my_addr->contact[i].age,
				my_addr->contact[i].tel,
				my_addr->contact[i].addr);
		}
	}
	printf("SearchName Over\n");
}

//修改联系人的信息
void revise_linkman(pLMCount my_addr)
{
	int ret = -1;//存放要修改的联系人的下标
	char revise_name[20] = "0";
	if (my_addr == NULL)//表示还没开始添加联系人
	{
		printf("Not have linkman,please enter linkman again.\n");
		return;
	}
	if (my_addr->sz == 0) //表示将联系人清空之后的情况，my_addr的地址存在，但是没有联系人了
	{
		printf("Not have linkman to revise.\n");
		return;
	}
	printf("Please scanf name that you want revise:>");
	scanf("%s", revise_name);
	ret = find_people_name(my_addr, revise_name);
	if (ret != -1) //找到名字为revise_name的联系人对其进行更改信息
	{
		printf("Please scanf name:>");
		scanf("%s", my_addr->contact[ret].name);
		printf("Please sacnf sex:>");
		scanf("%s", my_addr->contact[ret].sex);
		printf("Please sacnf age:>");
		scanf("%d", &my_addr->contact[ret].age);
		printf("Please sacnf tel:>");
		scanf("%s", my_addr->contact[ret].tel);
		printf("Please sacnf addr:>");
		scanf("%s", my_addr->contact[ret].addr);
	}
	else //没有找到要更改的联系人信息，结束
	{
		printf("Not found need revise_name\n");
		return;
	}
	printf("Revise_Name Over\n");
}
```

## test.c

``` c
#include "address.h"

void menu()
{
	printf("******       ADDRESS LIST     *****\n");
	printf("******  1.enter     2.empty   *****\n");
	printf("******  3.delete    4.sort    *****\n");
	printf("******  5.show      6.search  *****\n");
	printf("******  7.revise    8.exit    *****\n");
	printf("***********************************\n");
}
int main()
{
	LMCount addr;
	pLMCount my_addr = &addr;
	init_linkman(my_addr);
	int input = 0;
	do
	{
		menu();
		printf("Please choose function:>");
		scanf("%d", &input);
		switch (input)
		{
		case ENTER:
			enter_linkman(my_addr);
			break;
		case EMPTY:
			empty_linkman(my_addr);
			break;
		case DELETE:
			delete_linkman(my_addr);
			break;
		case SORT:
			sort_linkman(my_addr);
			break;
		case SHOW:
			show_linkman(my_addr);
			break;
		case SEARCH:
			search_linkman(my_addr);
			break;
		case REVISE:
			revise_linkman(my_addr);
			break;
		case EXIT:
			save_linkman(my_addr);
			free(my_addr->contact);
			my_addr->contact = NULL;
			exit(0);
			break;
		default:
			printf("Choose is error,please again choose.\n");
			break;
		}
	} while (input);
	

	return 0;
}
```

